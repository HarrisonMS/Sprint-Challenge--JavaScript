...
const tyranno = {
  ...
  speak: function() {
    return 'Ello M8 did ya know im British!!';
  },
};
...
const unis = graduates.map(unis => unis.university).sort();
  console.log(unis);
...
const contactInfo = [];
graduates.forEach(function(c){
  return contactInfo.push(`${c.first_name} ${c.email}`);
});
...
*/
const populationTotal = zooAnimals.reduce(function(totalPop, item) {
